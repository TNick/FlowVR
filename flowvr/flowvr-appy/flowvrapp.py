...
class Port:
  ...
  def __init__(self, owner, name, direction='out', messagetype='full', blockstate='blocking'):
    ...
    self.blockstate = blockstate   # 'blocking'
...
class FlowvrRunSSH(Run):
  ...
  def __init__(self, cmdline, name = '', host = '', cores = ''):
...
class FlowvrRunSSHMultiple(Run):
  ...
  def __init__(self, cmdline, prefix, hosts, cores = ''):
    self.cmdline = cmdline
    self.name = prefix
...
class FlowvrRunOpenMPI(Run):
  ...
  def get_cmdline(self):
    return "mpirun %s --host %s %s" % (self.options, self.hosts, self.cmdline)
...
class Primitive(Component):
  ...
  def addPort(self, name, direction='out', messagetype='full'):
    assert name not in self.ports, "port %s already exists on %s" % (name, self)
    port = self.ports[name] = Port(self, name, direction, messagetype)
    return port 
