...
class Greedy(flowvrapp.Component):
  ...
  def __init__(self, prefix, filter_class = FilterIt, host = ''):    
    ...
    self.filter, self.synchrnizer, self.presignal = filt, sync, ps
...
class GreedyMultiple(flowvrapp.Component):
  ...
  def newPortPair(self, host = '', filter_class = FilterIt):
    ...
    filt = filter_class(self.prefix + "/filters/%d" % self.nfilter)
...
class SpyModule(flowvrapp.Module):
  ...
  def __init__(self, name, messagetype = 'full'):    
    flowvrapp.Module.__init__(self, name, cmdline = "python -m spy_module.py")
