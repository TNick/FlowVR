...
static void idle_func ( void )
{
  ...
  // OUTPUT POSITION
  {
    ...
    // Temporary variable used to hold the transformed mouse position
    float mousePosition[2];
    mousePosition[0] = ((NX==0)?1.0f:(float)(NX-1)/NX)*((float)(mx))/win_x;
    mousePosition[1] = ((NY==0)?1.0f:(float)(NY-1)/NY)*((float)(win_y-1-my))/win_y;

    // Add the mouse event in the chunk list
    mouseMsgs->addEventMouse(mouse_buttons,mousePosition);

    // Finally, send the message :
    mouseMsgs->put(&portPosition);
  }

  // NEXT ITERATION
  if (!module->wait()) exit(1);
...
int main ( int argc, char ** argv )
{
  ...
  if (inputVelocity) printf ( "\t Toggle density/velocity display with the 'v' key\n" );
