...
class ModuleAPI
{
 public:
  ...
  ///
  /// Note: The message buffer cannot be modified by the module.
  /// It should also be freed when not needed anymore. Alternatively,
  /// this method frees any buffer attached to the given message
  ...
  /// @name Topology
  /// @{
  virtual topo_cpuset_t  getCPUMask() const;
  virtual bool           setCPUMask(topo_cpuset_t mask, const Topo &topo = Topo(false) );
  ...
  /// This method can be used to log the module PID in ~/.flowvr dir.
  void writePID(const std::string &name = std::string(""));
  /// @}

 protected:
  Topo topology;
  topo_cpuset_t cpumask;
