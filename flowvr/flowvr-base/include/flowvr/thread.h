...
class Thread
{
 public:
   ...
   */
  eAffinityError setAffinity( topo_cpuset_t mask, const Topo &topo = Topo(false) );

  /**
   * @returns the current cpu affinity mask (a zero-mask if none was set before)
   */
  topo_cpuset_t getAffinity() const;
 ...
 protected:
  ...
  topo_cpuset_t  thread_cpu_mask; /** the cpu mask as set, for getAffinity() */
