...
{
	// clear mask properly
	topo_cpuset_zero( &thread_cpu_mask );
}
...
Thread::eAffinityError Thread::setAffinity( topo_cpuset_t mask, const Topo &topo )
{
	...
	if(tID)
	{
		...
		{
			// ok, now we can bind
			if(topo_set_cpubind(sys_topology.topology.t_topology, &mask, TOPO_CPUBIND_THREAD ) == 0)
			{
				// save the current value of the mask set by the user
				// to match our internal state (for getAffinity()).
				memcpy(&thread_cpu_mask,&mask, sizeof(topo_cpuset_t));
...
topo_cpuset_t Thread::getAffinity() const
{
