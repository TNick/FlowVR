...
flowvr::utils::Option<std::string> OptName      ("name"      , 'n' , "Set the daemon internal name", std::string("FlowVR-Daemon 2.1.0"),false);
...
flowvr::utils::Option<std::string> OptNet       ("network"       , '\0', "NET class", std::string("flowvr.plugins.NetTCP"),false);
...
#define FLOWVR_STR2(f) FLOWVR_STR(f)
  ...
  std::cout << "FlowVR Daemon version "<< FLOWVR_STR2(FLOWVR_DAEMON_VERSION);
...

  {
    ...
    char buf[16];
    snprintf(buf,sizeof(buf),"%zd",OptMemID.value());
