...
flowvr::utils::FlagOption OptSync("sync", 's', "Sync mode (flush between each command)");
...
flowvr::utils::FlagOption OptEnd("end", '\0', "Stop application when one module ends.");
...
void print_modules(BasicController *bc) {
  ...
  for(std::set<std::string>::iterator it=bc->modules.begin(); it!=bc->modules.end(); it++) {
    std::cout << *it << " " ;
  } 
  std::cout << "]\n";
}
...
std::string BasicController::getcmdline()
{
	std::string res;
	int c;
	static int nchar = 0;
