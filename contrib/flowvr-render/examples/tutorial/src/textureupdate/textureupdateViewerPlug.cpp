...
{
	args["IMAGELIST"] = Parameter( "light0.png, light1.png, light2.png", "List of filenames to load as textures.", Parameter::P_LIST, Parameter::PS_STRING );
}
...
namespace
{
	...
	public:
		...
		, idT1(0)
		{
			try
			{
				Parameter::ParListS images = args("IMAGELIST").getValue<Parameter::ParListS>();
				for( Parameter::ParListS::const_iterator it = images.begin(); it != images.end(); ++it )
				{
					ChunkTexture *texture = textures.loadTexture( 0, "images/" + *it );
					if( texture )
						m_vTextures.push_back(texture);
					else
						std::cerr << "could not load texture [" << *it << "]" << std::endl;
				}

				m_it = m_vTextures.end();
			}
			catch( std::exception & )
			{
		...
		virtual bool once(flowvr::ModuleAPI &module, flowvr::MessageWrite &mw, flowvr::StampList *sl, flowvr::Allocator &alloc )
		{
			...
			ID idT0    = module.generateID(); // Id of the Texture0
			   idT1    = module.generateID(); // Id of the Texture1
			...
			texture = scene.loadTexture(idT1, "images/light0.png");
			if(texture)
			{
				texture->level = -1; // no level of detail used
				texture->gen = -1;   // this is a static resource
                                      ...
                                      flowvr::Allocator &allocate )
		{
			...
			if( m_it == m_vTextures.end() )
				m_it = m_vTextures.begin();

			ChunkRenderWriter scene;

			const ChunkTexture *tex = *m_it++;

			ChunkTextureUpdate *update = scene.updateTexture( idT1, (*tex).imageType, (*tex).pixelType, (*tex).nx, (*tex).ny, (*tex).nz, 0, 0, 0, (*tex).nx, (*tex).ny, (*tex).nz );

			memcpy( (void*)update->data(), tex->data(), tex->lineSize * tex->ny );

			out = scene.dump<Allocator>( &allocate );
		...
		ID  idT1;
		std::vector<ChunkTexture*> m_vTextures;
		std::vector<ChunkTexture*>::const_iterator m_it;
		ChunkRenderWriter textures;
	};
}


DEFIMP_PORTPLUG_NOSERVICE( multitextureViewerPlugHandler, multitextureViewerPlug )
