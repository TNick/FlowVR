...
public:
	...
	enum
	{
		SMOOTHGROUP_SHIFT = 20
	};

	int merge;

	MeshObjData(Mesh* m) :
		mesh(m), v0(0), merge(0), smoothgroup(0)
	{
	}
	...
	void addMaterial(const Mesh::Material& m)
	{
		...
		if (dest)
			*dest = m;
			...
			dest = new Mesh::Material(m);
		std::cout << "Material " << m.matname << " added." << std::endl;
	}

	void addMatGroup(const std::string& matname, const std::string& gname,
			int f0, int nbf)
	{
		if (nbf <= 0)
			return;
		...
		matg.f0 = f0;
		matg.nbf = nbf;
		std::cout << "Group " << gname << " material " << matname << " f0="
				<< matg.f0 << " nbf=" << matg.nbf << std::endl;
		if (!matname.empty())
		{
			...
			{
				std::cerr << "ERROR: material " << matname << " not found."
						<< std::endl;
			}
		}
	...
	Vec3f readColor(const char* c)
	{
		...
		if (*(c = skip(c)))
		{
			res.x() = strtof(skip(c), (char**) &c);
			if (*(c = skip(c)))
			{
				res.y() = strtof(skip(c), (char**) &c);
				if (*(c = skip(c)))
				{
					res.z() = strtof(skip(c), (char**) &c);
				}
				else
					res.z() = res.y();
			}
			...
			const std::string& basefile)
	{
		...
		// remove any ".."
		while ((filename.substr(0, 3) == "../" || filename.substr(0, 3)
				== "..\\") && !base.empty())
		{
	...
	bool loadmtl(const char* filename, const char* basefilename)
	{
		...
		if (fp == NULL)
		{
			std::cerr << "ERROR: Loading MTL file " << fpath << " failed."
					<< std::endl;
			return false;
		}
		...
		while (fgets(line, sizeof(line), fp) != NULL)
		{
			...
			{ // specular power
				mat.shininess = strtof(skip(c), (char**) &c);
			}
			else if (t == "Tf")
			...
			{ // dissolve
				mat.alpha = strtof(skip(c), (char**) &c);
			}
			else if (t == "illum")
			{
				int i = atoi(skip(c));
			...
			{
				std::cerr << "ERROR: MTL parsing on line " << l << ": " << line
						<< std::endl;
			}
		}
		addMaterial(mat);
	...
	{
		int iv = strtol(skip(*c), (char**) c, 10);
		if (iv < 0)
			iv += t_v.size();
		else
			iv += v0;
	...
	bool load(const char* filename, const char* filter)
	{
		...
		{
			int len = 0;
			sscanf(filter, "<%f,%f,%f>-<%f,%f,%f>%n", &bb.a[0], &bb.a[1],
					&bb.a[2], &bb.b[0], &bb.b[1], &bb.b[2], &len);
			filter += len;
		}
		...
		while (fgets(line, sizeof(line), fp) != NULL)
		{
			...
			case '#':
				if (!strncmp(line, "#Vertex Count ", strlen("#Vertex Count ")))
				{
					...
					std::cout << "Allocating " << n << " vertices."
							<< std::endl;
					t_v.reserve(n + 1);
					//t_v_g.reserve(n+1);
				}
				...
				case ' ': // face
				{
					if (inc)
					{
						...
						while (*c != '\n' && *c != '\0' && *c != '\r')
						{
							...
							if (bb.isEmpty() || (bb.in(t_v[fp0[0]]) && bb.in(
									t_v[fp1[0]]) && bb.in(t_v[fp2[0]])))
							{
								t_fp.push_back(fp0);
								t_fp.push_back(fp1);
								t_fp.push_back(fp2);
								// put the position index temporarily in the mesh face array
								mesh->FP(mesh->nbf()) = Vec3i(fp0[0], fp1[0],
										fp2[0]);
								//mesh->FP(mesh->nbf())=f;
							}
							fp1 = fp2;
			...
			case 'g':
				...
				f0 = mesh->nbf();
				gname = skip(c);
				inc = (!filter || !strncmp(gname.c_str(), filter,
						strlen(filter)) || !strncmp(matname.c_str(), filter, strlen(filter)));
				break;
			case 's':
				++c;
		...
		// The map store the final index of each combination
		...
		for (unsigned int i = 0; i < t_fp.size(); i++)
		{
			...
			{
				std::cerr << "ERROR: invalid vertex index " << fp0[0] << " > "
						<< inputnbp - 1 << std::endl;
				t_fp[i][0] = fp0[0] = 0;
			}
		...
		for (int i = 0, j = 0; i < inputnbp; i++)
		{
			...
			for (std::map<std::pair<int, int>, int>::iterator it =
					vertNormTexMap[i].begin(); it != vertNormTexMap[i].end(); ++it)
			{
				mesh->points_p[j] = p;
				...
				if (mesh->getAttrib(Mesh::MESH_POINTS_NORMAL))
					mesh->points_n[j] = t_vn[(n
							& ((1 << SMOOTHGROUP_SHIFT) - 1))];
		...
		fclose(fp);
		std::cout << "Loaded file " << filename << std::endl;
		std::cout << t_v.size() - 1 << " positions, " << t_vt.size() - 1
				<< " texcoords, " << t_vn.size() - 1 << " normals" << std::endl;
		std::cout << mesh->nbp() << " final points, " << mesh->nbf()
				<< " triangles, " << mesh->nbg() << " normal groups"
				<< std::endl;
...
bool Mesh::saveObj(FILE* fp, int& v0, int &vn0, int &vt0) const
{
	...
	for (std::map<std::string, Material*>::const_iterator it = mat_map.begin(),
			itend = mat_map.end(); it != itend; ++it)
	{
	...
	for (int i = 0; i < nbf(); i++)
	{
		if (i >= mat_f1 && mat + 1 < mat_groups.size())
		{
			++mat;
